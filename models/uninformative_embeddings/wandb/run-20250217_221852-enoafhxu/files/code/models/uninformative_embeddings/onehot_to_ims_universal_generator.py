...
#%%
device = f.set_up_gpu()
x_train, y_train, train_chem_encodings_tensor, train_indices_tensor = f.create_dataset_tensors_for_generator(train_spectra, train_embeddings, device)
del train_spectra
train_data = TensorDataset(x_train, train_chem_encodings_tensor, y_train, train_indices_tensor)

x_val, y_val, val_chem_encodings_tensor, val_indices_tensor = f.create_dataset_tensors_for_generator(val_spectra, val_embeddings, device)
del val_spectra
val_data = TensorDataset(x_val, val_chem_encodings_tensor, y_val, val_indices_tensor)

x_test, y_test, test_chem_encodings_tensor, test_indices_tensor = f.create_dataset_tensors_for_generator(test_spectra, test_embeddings, device)
del test_spectra
...
f.train_generator(
    ...
    model_type='onehot_to_ims_generator'
    )